...
Public Class SFMLHScrollbar
    ...
    Public ReadOnly Property ValuePercentPixel As Single
        Get
            ...
            Return ((Value / (Maximum - Minimum)) - (Minimum / (Maximum - Minimum))) * (Size.Width - DotSize)
        End Get
    End Property
    ...
    Public ReadOnly Property DotSize As Integer
        Get
            Return Width / ((Maximum - Minimum) / Width)
        End Get
    End Property

    Public Function GetValueFromClick(ByVal p As Point) As Integer
        Dim v As Integer
        v = ((p.X - Left - DotSize / 2) * 100 / (Width - DotSize)) * (Maximum - Minimum) / 100 + Minimum
        If v < Minimum Then
            Return Minimum
    ...
    Public Sub Draw(ByRef w As RenderWindow) Implements ISFMLControl.Draw
        If Visible Then
            ...
            UpSq = New RectangleShape(New Vector2f(Height, Height))
            UpSq.Position = New Vector2f(Left - Height, Top)
            UpSq.FillColor = ArrowColor
            UpSq.OutlineColor = BorderColor
            UpSq.OutlineThickness = -1

            DownSq = New RectangleShape(New Vector2f(Height, Height))
            DownSq.Position = New Vector2f(Left + Width, Top)
            DownSq.FillColor = ArrowColor
            DownSq.OutlineColor = BorderColor
            DownSq.OutlineThickness = -1

            dot = New RectangleShape(New Vector2f(DotSize, Height - 2))
            dot.Position = New Vector2f(Location.X + ValuePercentPixel, Location.Y + 1)
            dot.FillColor = DotColor
            dot.OutlineColor = BorderColor
            dot.OutlineThickness = -1

            UpArr = New CircleShape(Height / 2, 3)
            ...
            DownArr = New CircleShape(Height / 2, 3)
            ...
            Dim t As New Text(Value, SFMLFont, SFMLFontSize)
            t.Position = New Vector2f(Location.X + 30, Location.Y + 30)
            t.Color = SFML.Graphics.Color.Black
